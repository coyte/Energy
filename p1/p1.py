...
MySQL_loaded = True
try:
#    import mysql.connector changed the line to reflect the pymysql
    import pymysql
except ImportError:
    MySQL_loaded=False
...
#    print(query)
    try:
        ...
        db = pymysql.connect(host=p1_mysql_host, port=3306, user=p1_mysql_user, passwd=p1_mysql_passwd, db=p1_mysql_db)
        c = db.cursor()
        c.execute (query)
        db.commit()
